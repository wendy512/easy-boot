...
public class DateUtils {
    ...
    public static Date strToDate(String strTime) {
        if (strTime == null || strTime.trim().length() <= 0) return null;
        ...
        for (Iterator iter = list.iterator(); iter.hasNext();) {
            ...
            if (strTime.indexOf("-") > 0 && format.indexOf("-") < 0) continue;
            if (strTime.indexOf("-") < 0 && format.indexOf("-") > 0) continue;
            if (strTime.length() > format.length()) continue;
    ...
    public static boolean validateIsDate(String strTime) {
        if (strTime == null || strTime.trim().length() <= 0) return false;
     ...
     * @return 计算后的日期
     */
    public static Date addDate(Date date, int year, int month, int day, int hour, int minute,
    ...
    */
    ...
    public static Date formatStr(String source, String pattern){
    ...
    public static Long formatStringUTCtime(String receiveTimestamp) {
		String format = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";
		Date date = null;
		Long receiveTime = null;
		if(StringUtils.isNotBlank(receiveTimestamp)) {
			SimpleDateFormat sdf = new SimpleDateFormat(format);
			 try {
				date = sdf.parse(receiveTimestamp);
		        Calendar calendar = Calendar.getInstance();
		        calendar.setTime(date);
		        calendar.set(Calendar.HOUR, calendar.get(Calendar.HOUR) + 8);
		         receiveTime = calendar.getTimeInMillis()*1000;
			} catch (ParseException e) {
			}
		}
		return receiveTime;
