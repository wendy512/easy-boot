...
public final class SerializeUtil {
    ...
    public static <T> byte[] serialize(Serializer serializer, T t) {
        ...
        byte[] source = serializer.serialize(t);
        try {
            return Snappy.compress(appendClassName(t, source));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    public static  <T> T deserialize(Serializer serializer, byte[] bytes) {
        if (null == bytes) {
            return null;
        }
        ...
        try {
            source = Snappy.uncompress(bytes);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public static Tuple2<String,byte[]> subClassName(byte[] source) {
        byte[] header = Arrays.copyOf(source, 4);
        int classNameLength = byteArrayToInt(header);
        byte[] classNameBytes = Arrays.copyOfRange(source, 4, 4 + classNameLength);
        String className = new String(classNameBytes, StandardCharsets.UTF_8);
        byte[] data = Arrays.copyOfRange(source, 4 + classNameLength, source.length);
