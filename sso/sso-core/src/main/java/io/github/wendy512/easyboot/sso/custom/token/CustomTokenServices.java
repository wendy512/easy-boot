...
package io.github.wendy512.easyboot.sso.custom.token;
...
import io.github.wendy512.easyboot.common.util.UUIDUtils;
import io.github.wendy512.easyboot.sso.custom.user.UserInfo;

/**
 * 拓展token service
 * @author wendy512
 * @date 2022-05-26 16:01:26
 * @since 1.0.0
 */
public class CustomTokenServices extends DefaultTokenServices
    ...
    private OAuth2RefreshToken createRefreshToken(OAuth2Authentication authentication) {
        ...
        String value = UUIDUtils.randomUUID();
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        DefaultOAuth2AccessToken token = new DefaultOAuth2AccessToken(generateToken(authentication, refreshToken));
    ...
    protected String generateToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        StringBuilder tokenInfo = new StringBuilder();
        tokenInfo.append(System.currentTimeMillis())
                 .append(refreshToken.getValue());
        
        if (authentication.getPrincipal() instanceof UserInfo) {
            UserInfo userInfo = (UserInfo) authentication.getPrincipal();
            tokenInfo.append(userInfo.getUsername())
                     .append(userInfo.getId());
        } else {
            tokenInfo.append(authentication.getPrincipal().toString());
        }
        return DigestUtils.sha256Hex(tokenInfo.toString()) + "-" + VERSION;
    }
